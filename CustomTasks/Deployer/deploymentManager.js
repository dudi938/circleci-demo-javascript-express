...
function main() {
    ...
    switch (action) {
        ...
        case RUN_REPLICATION_SET:
            ...
            execCommand('sudo ssh -i ' + PRIVATE_KEY_PATH + '   -oStrictHostKeyChecking=no  testim@' + gridsPublicIP[0] + ' sh   /home/testim/configureReplicaset.sh' + ' ' + gridsPublicIP[0] + ' ' + gridsPublicIP[1] + ' ' + gridsPublicIP[2], function () {
            });
            break;
        case RUN_EDIT_CAMELOT_PROPERTIES:
            var gridsPublicIP = fs.readFileSync(GRID_IP).toString().split('\r\n');

            execCommand('sudo ssh -i ' + PRIVATE_KEY_PATH + '   -oStrictHostKeyChecking=no  testim@' + gridsPublicIP[0] + ' sh   /home/testim/editCamelotProperties.sh' + ' ' + gridsPublicIP[0] + ' ' + gridsPublicIP[1] + ' ' + gridsPublicIP[2], function () {
                execCommand('sudo ssh -i ' + PRIVATE_KEY_PATH + '   -oStrictHostKeyChecking=no  testim@' + gridsPublicIP[1] + ' sh   /home/testim/editCamelotProperties.sh' + ' ' + gridsPublicIP[0] + ' ' + gridsPublicIP[1] + ' ' + gridsPublicIP[2], function () {
                    execCommand('sudo ssh -i ' + PRIVATE_KEY_PATH + '   -oStrictHostKeyChecking=no  testim@' + gridsPublicIP[2] + ' sh   /home/testim/editCamelotProperties.sh' + ' ' + gridsPublicIP[0] + ' ' + gridsPublicIP[1] + ' ' + gridsPublicIP[2], function () {
                    });
                });
            });
            break;
